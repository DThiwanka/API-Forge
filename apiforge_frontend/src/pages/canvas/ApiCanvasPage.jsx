...
import ApiCanvas from '../../features/canvas/components/ApiCanvas';
import { useWorkspacesQuery } from '../../features/workspace/hooks/useWorkspace';
import useWorkspaceStore from '../../features/workspace/store/workspaceStore';
import { Terminal, ArrowRight, Loader2 } from 'lucide-react';

export default function ApiCanvasPage() {
  ...
  // If on /workspace/:workspaceId/canvas without valid id, redirect to first workspace
  useEffect(() => {
    if (!routeWorkspaceId && workspaces.length > 0) {
      navigate(`/workspace/${workspaces[0].id}/canvas`, { replace: true });
    }
      ...
      <AppShell>
        <div className="flex-1 flex flex-col items-center justify-center p-8 text-center select-none bg-[#090a0f]">
          ...
          <p className="text-xs text-slate-400 max-w-sm mb-4">
            You must be signed in to access the APIForge Spatial Canvas.
          </p>
          <button
            type="button"
        ...
        <div className="flex-1 flex flex-col min-w-0 h-full overflow-hidden bg-[#090a0f]">
          {loadingWorkspaces ? (
            <div className="flex-1 flex flex-col items-center justify-center p-8 text-slate-400">
              ...
              <span className="text-xs font-mono">Loading workspace canvas...</span>
            </div>
          ) : (
            <ApiCanvas workspaceId={currentWorkspaceId} />
          )}
        </div>
      </div>
    </AppShell>
