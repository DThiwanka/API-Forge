...
import ExecutionErrorCard from './ExecutionErrorCard';
...
export default function ResponseInspector({
  ...
  return (
    ...
    >
      ...
      {/* RE-EXECUTING BANNER (Preserves previous response without UI flicker) */}
      {status === 'loading' && response && (
        <div className="flex items-center justify-between px-4 py-1.5 bg-sky-950/70 border-b border-sky-500/30 text-xs font-mono text-sky-200 select-none">
          <div className="flex items-center gap-2">
            <Loader2 size={13} className="animate-spin text-sky-400" />
            <span className="font-semibold">Sending request...</span>
          </div>
      ...
      {/* IDLE STATE (when no response has ever been received) */}
      ...
      {/* FOCUSED LOADING STATE (when no previous response exists) */}
      {status === 'loading' && !response && (
      ...
      {/* SUCCESS OR PRESERVED RESPONSE CONTENT */}
      {response && (status === 'success' || status === 'loading') && (
        <div className={cn('flex-1 flex flex-col min-h-0 overflow-hidden', status === 'loading' && 'opacity-60 pointer-events-none transition-opacity')}>
          <ResponseTabs
            activeTab={activeTab}
